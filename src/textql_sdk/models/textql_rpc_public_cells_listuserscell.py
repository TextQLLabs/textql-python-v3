"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .textql_rpc_public_cells_agentinfo import (
    TextqlRPCPublicCellsAgentInfo,
    TextqlRPCPublicCellsAgentInfoTypedDict,
)
import pydantic
from pydantic import model_serializer
from textql_sdk.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from typing import List, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class TextqlRPCPublicCellsListUsersCellTypedDict(TypedDict):
    search_term: NotRequired[str]
    user_type: NotRequired[str]
    r"""\"all\", \"bots\", or \"humans\" """
    total_count: NotRequired[int]
    sandbox_available: NotRequired[bool]
    error_message: NotRequired[Nullable[str]]
    agents: NotRequired[List[TextqlRPCPublicCellsAgentInfoTypedDict]]


class TextqlRPCPublicCellsListUsersCell(BaseModel):
    search_term: Annotated[Optional[str], pydantic.Field(alias="searchTerm")] = None

    user_type: Annotated[Optional[str], pydantic.Field(alias="userType")] = None
    r"""\"all\", \"bots\", or \"humans\" """

    total_count: Annotated[Optional[int], pydantic.Field(alias="totalCount")] = None

    sandbox_available: Annotated[
        Optional[bool], pydantic.Field(alias="sandboxAvailable")
    ] = None

    error_message: Annotated[
        OptionalNullable[str], pydantic.Field(alias="errorMessage")
    ] = UNSET

    agents: Optional[List[TextqlRPCPublicCellsAgentInfo]] = None

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(
            [
                "searchTerm",
                "userType",
                "totalCount",
                "sandboxAvailable",
                "errorMessage",
                "agents",
            ]
        )
        nullable_fields = set(["errorMessage"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))
            is_nullable_and_explicitly_set = (
                k in nullable_fields
                and (self.__pydantic_fields_set__.intersection({n}))  # pylint: disable=no-member
            )

            if val != UNSET_SENTINEL:
                if (
                    val is not None
                    or k not in optional_fields
                    or is_nullable_and_explicitly_set
                ):
                    m[k] = val

        return m


try:
    TextqlRPCPublicCellsListUsersCell.model_rebuild()
except NameError:
    pass
