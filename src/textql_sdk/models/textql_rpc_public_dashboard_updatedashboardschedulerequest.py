"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .textql_rpc_public_dashboard_datasource import (
    TextqlRPCPublicDashboardDataSource,
    TextqlRPCPublicDashboardDataSourceTypedDict,
)
import pydantic
from pydantic import model_serializer
from textql_sdk.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from typing import List, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class TextqlRPCPublicDashboardUpdateDashboardScheduleRequestTypedDict(TypedDict):
    dashboard_id: NotRequired[str]
    schedule_enabled: NotRequired[bool]
    cron_string: NotRequired[Nullable[str]]
    data_sources: NotRequired[List[TextqlRPCPublicDashboardDataSourceTypedDict]]


class TextqlRPCPublicDashboardUpdateDashboardScheduleRequest(BaseModel):
    dashboard_id: Annotated[Optional[str], pydantic.Field(alias="dashboardId")] = None

    schedule_enabled: Annotated[
        Optional[bool], pydantic.Field(alias="scheduleEnabled")
    ] = None

    cron_string: Annotated[
        OptionalNullable[str], pydantic.Field(alias="cronString")
    ] = UNSET

    data_sources: Annotated[
        Optional[List[TextqlRPCPublicDashboardDataSource]],
        pydantic.Field(alias="dataSources"),
    ] = None

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(
            ["dashboardId", "scheduleEnabled", "cronString", "dataSources"]
        )
        nullable_fields = set(["cronString"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))
            is_nullable_and_explicitly_set = (
                k in nullable_fields
                and (self.__pydantic_fields_set__.intersection({n}))  # pylint: disable=no-member
            )

            if val != UNSET_SENTINEL:
                if (
                    val is not None
                    or k not in optional_fields
                    or is_nullable_and_explicitly_set
                ):
                    m[k] = val

        return m


try:
    TextqlRPCPublicDashboardUpdateDashboardScheduleRequest.model_rebuild()
except NameError:
    pass
