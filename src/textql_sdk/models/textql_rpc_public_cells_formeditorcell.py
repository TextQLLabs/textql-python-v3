"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .textql_rpc_public_cells_editableform import (
    TextqlRPCPublicCellsEditableForm,
    TextqlRPCPublicCellsEditableFormTypedDict,
)
from .textql_rpc_public_cells_formeditoraction import (
    TextqlRPCPublicCellsFormEditorAction,
)
import pydantic
from pydantic import model_serializer
from textql_sdk.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from typing import Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class TextqlRPCPublicCellsFormEditorCellTypedDict(TypedDict):
    action: NotRequired[TextqlRPCPublicCellsFormEditorAction]
    form_snapshot: NotRequired[TextqlRPCPublicCellsEditableFormTypedDict]
    form: NotRequired[TextqlRPCPublicCellsEditableFormTypedDict]
    form_id: NotRequired[Nullable[str]]


class TextqlRPCPublicCellsFormEditorCell(BaseModel):
    action: Optional[TextqlRPCPublicCellsFormEditorAction] = None

    form_snapshot: Annotated[
        Optional[TextqlRPCPublicCellsEditableForm], pydantic.Field(alias="formSnapshot")
    ] = None

    form: Optional[TextqlRPCPublicCellsEditableForm] = None

    form_id: Annotated[OptionalNullable[str], pydantic.Field(alias="formId")] = UNSET

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["action", "formSnapshot", "form", "formId"])
        nullable_fields = set(["formId"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))
            is_nullable_and_explicitly_set = (
                k in nullable_fields
                and (self.__pydantic_fields_set__.intersection({n}))  # pylint: disable=no-member
            )

            if val != UNSET_SENTINEL:
                if (
                    val is not None
                    or k not in optional_fields
                    or is_nullable_and_explicitly_set
                ):
                    m[k] = val

        return m


try:
    TextqlRPCPublicCellsFormEditorCell.model_rebuild()
except NameError:
    pass
