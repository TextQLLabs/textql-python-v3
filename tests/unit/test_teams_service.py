"""Unit tests for the Teams service (sdk.teams)."""
...
PATH_PREFIX = "/textql.rpc.public.teams.TeamsService"
...
def test_create_uuid_sync(make_sdk):
    ...
    resp = bundle.sdk.teams.create_uuid(body={})

    assert_common(bundle, "CreateTeamsUuid")
    assert resp.uuid == "uuid-1"


async def test_create_uuid_async(make_sdk):
    bundle = make_sdk(lambda req: json_response(200, {"uuid": "uuid-2"}))
    resp = await bundle.sdk.teams.create_uuid_async(body={})
    assert_common(bundle, "CreateTeamsUuid")
...
def test_create_uuid_404_raises(make_sdk):
    ...
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        bundle.sdk.teams.create_uuid(body={})
...
async def test_create_uuid_async_500_raises(make_sdk):
    ...
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        await bundle.sdk.teams.create_uuid_async(body={})
...
def test_delete_installation_sync(make_sdk):
    ...
    resp = bundle.sdk.teams.delete_installation(tenant_id="tenant-1")

    assert_common(bundle, "DeleteInstallation")
    assert bundle.transport.body_json()["tenantId"] == "tenant-1"
    assert resp is not None


async def test_delete_installation_async(make_sdk):
    bundle = make_sdk(lambda req: json_response(200, {}))
    resp = await bundle.sdk.teams.delete_installation_async(tenant_id="tenant-2")
    assert_common(bundle, "DeleteInstallation")
    assert resp is not None
...
def test_delete_installation_404_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(404, {"message": "not found"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        bundle.sdk.teams.delete_installation(tenant_id="missing")
    assert exc_info.value.status_code == 404
...
async def test_delete_installation_async_500_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(500, {"message": "boom"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        await bundle.sdk.teams.delete_installation_async(tenant_id="missing")
    assert exc_info.value.status_code == 500
...
def test_get_current_user_sync(make_sdk):
    ...
    resp = bundle.sdk.teams.get_current_user(body={})
...
async def test_get_current_user_async(make_sdk):
    ...
    resp = await bundle.sdk.teams.get_current_user_async(body={})
    assert_common(bundle, "GetCurrentUser")
...
def test_get_current_user_404_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(404, {"message": "not found"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        bundle.sdk.teams.get_current_user(body={})
    assert exc_info.value.status_code == 404
...
async def test_get_current_user_async_500_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(500, {"message": "boom"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        await bundle.sdk.teams.get_current_user_async(body={})
    assert exc_info.value.status_code == 500


# ---------------------------------------------------------------------------
# handle_o_auth_callback -- code/state placed in the JSON request BODY (not
# query params), per TextqlRPCPublicTeamsHandleTeamsOAuthCallbackRequest usage
# in src/textql_sdk/teams.py.
# ---------------------------------------------------------------------------


def test_handle_o_auth_callback_sync(make_sdk):
    bundle = make_sdk(
        lambda req: json_response(200, {"success": True, "tenantId": "tenant-1"})
    )

    resp = bundle.sdk.teams.handle_o_auth_callback(
        code="fake-oauth-code-123", state="fake-oauth-state-abc"
    )

    assert_common(bundle, "HandleTeamsOAuthCallback")
...
async def test_handle_o_auth_callback_async(make_sdk):
    ...
    resp = await bundle.sdk.teams.handle_o_auth_callback_async(
...
def test_handle_o_auth_callback_404_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(404, {"message": "not found"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        bundle.sdk.teams.handle_o_auth_callback(code="fake-oauth-code", state="fake-state")
    assert exc_info.value.status_code == 404
...
async def test_handle_o_auth_callback_async_500_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(500, {"message": "boom"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        await bundle.sdk.teams.handle_o_auth_callback_async(
...
def test_list_sync(make_sdk):
    ...
    resp = bundle.sdk.teams.list(body={})
...
async def test_list_async(make_sdk):
    ...
    resp = await bundle.sdk.teams.list_async(body={})
    assert_common(bundle, "ListChannels")
...
async def test_list_async_500_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(500, {"message": "boom"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        await bundle.sdk.teams.list_async(body={})
    assert exc_info.value.status_code == 500
...
def test_list_installations_sync(make_sdk):
    bundle = make_sdk(
        lambda req: json_response(200, {"installations": [{"tenantId": "tenant-1"}]})
    )

    resp = bundle.sdk.teams.list_installations(body={})
...
async def test_list_installations_async(make_sdk):
    ...
    resp = await bundle.sdk.teams.list_installations_async(body={})
    assert_common(bundle, "ListInstallations")
...
def test_list_installations_404_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(404, {"message": "not found"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        bundle.sdk.teams.list_installations(body={})
    assert exc_info.value.status_code == 404
...
async def test_list_installations_async_500_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(500, {"message": "boom"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        await bundle.sdk.teams.list_installations_async(body={})
    assert exc_info.value.status_code == 500
...
def test_list_users_sync(make_sdk):
    ...
    resp = bundle.sdk.teams.list_users(body={})
...
async def test_list_users_async(make_sdk):
    ...
    resp = await bundle.sdk.teams.list_users_async(body={})
    assert_common(bundle, "ListUsers")
    assert resp.users == []
...
def test_list_users_404_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(404, {"message": "not found"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        bundle.sdk.teams.list_users(body={})
    assert exc_info.value.status_code == 404
...
async def test_list_users_async_500_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(500, {"message": "boom"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        await bundle.sdk.teams.list_users_async(body={})
    assert exc_info.value.status_code == 500
...
# sync_workspace
# ---------------------------------------------------------------------------
# NOTE: SyncWorkspace takes a single `tenant_id: Optional[str]`, NOT a list of
# workspace ids -- confirmed by reading src/textql_sdk/teams.py's
# `sync_workspace` signature and its
# `TextqlRPCPublicTeamsTeamsSyncWorkspaceRequest(tenant_id=tenant_id)` body
# construction.
...
def test_sync_workspace_sync(make_sdk):
    ...
    resp = bundle.sdk.teams.sync_workspace(tenant_id="tenant-1")
...
def test_sync_workspace_unset_tenant_id_omitted(make_sdk):
    bundle = make_sdk(lambda req: json_response(200, {}))
    bundle.sdk.teams.sync_workspace()
    body = bundle.transport.body_json()
    assert body is None or "tenantId" not in body


async def test_sync_workspace_async(make_sdk):
    bundle = make_sdk(lambda req: json_response(200, {"queued": False}))
    resp = await bundle.sdk.teams.sync_workspace_async(tenant_id="tenant-2")
    assert_common(bundle, "SyncWorkspace")
...
def test_sync_workspace_404_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(404, {"message": "not found"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        bundle.sdk.teams.sync_workspace(tenant_id="missing")
    assert exc_info.value.status_code == 404
...
async def test_sync_workspace_async_500_raises(make_sdk):
    bundle = make_sdk(lambda req: json_response(500, {"message": "boom"}))
    with pytest.raises(errors.TextqlDefaultError) as exc_info:
        await bundle.sdk.teams.sync_workspace_async(tenant_id="missing")
    assert exc_info.value.status_code == 500
...
def test_retries_backoff_retries_then_succeeds(make_sdk, sequence_handler):
    ...
    resp = bundle.sdk.teams.sync_workspace(tenant_id="tenant-1", retries=retry_config)
...
async def test_retries_backoff_retries_then_succeeds_async(make_sdk, sequence_handler):
    ...
    resp = await bundle.sdk.teams.create_uuid_async(body={}, retries=retry_config)
...
def test_server_url_override(make_sdk):
    ...
    bundle.sdk.teams.sync_workspace(tenant_id="tenant-1", server_url=override_url)
...
def test_http_headers_override(make_sdk):
    ...
    bundle.sdk.teams.sync_workspace(
        tenant_id="tenant-1", http_headers={"X-Custom-Header": "custom-value"}
    )
...
def test_timeout_ms_override_does_not_break_request(make_sdk):
    ...
    bundle.sdk.teams.sync_workspace(tenant_id="tenant-1", timeout_ms=5000)
